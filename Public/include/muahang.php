<?php 
    ...
    if($muahang->checkAddress($user) != false) {
        if($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $time = time();
            $ngaydh = date('Y-m-d h:i:s',$time+18000);
            $ngaygh = date('Y-m-d',strtotime(' + 4 day'));
                
            $result = $muahang->muahang($ma_HD,$makh,$thanhtoan,$ngaydh,$ngaygh,$trangthai);
        }
    } else {
    ...
    <form action="giohang.php?action=dathang&user=<?php echo $_SESSION['loginkh'] ?>" method = "POST">
        <div class="DCNH">
            <div class="DCNH-item jtf-content-spbw">
                ...
                ?>
                    ...
                    <div style = "flex: 1;">
                        <a style = "float: right;" href="?action=tddcgh" class = "btn bg-color white-color">Thay đổi địa chỉ</a>
                    </div>
                <?php } else { ?>
                            <div>
                                <i class="fa-solid fa-location-dot icon-location"></i>
                            ...
                            <div>
                                <a href="?action=dcgh" class = "btn bg-color white-color">Thêm địa chỉ nhận hàng</a>
                            </div>
                        <?php } ?>
            </div>
        </div>
        ...
        <div class="hoadon">
            <div class="body-hoadon">
                ...
                <div>
                    <button style="width: 100%;" class="btn bg-color white-color margin-top" type="submit" name="muahang">Mua hàng</button>
                </div>
            </div>
        </div>
    </form>
